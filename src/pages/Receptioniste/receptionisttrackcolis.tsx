...
const SuiviColisReceptionnistPage = () => {
  ...
  const handleAddSuivi = async () => {
    ...
    try {
      const response = await api.post(`/colis/${selectedColis.code_suivi}`, newSuivi);
      ...
      setSelectedColis(prev => ({
        ...
        suivis: [...(prev?.suivis || []), response.data]
      }));
      ...
      setError(err.response?.data?.message || err.message || 'Erreur lors de la mise à jour du suivi');
    } finally {
      setLoading(prev => ({...prev, submit: false}));
    }
  };

  const getNextPossibleStatuses = (): string[] => {
    if (!selectedColis?.statut) return [];
    const currentIndex = statusHierarchy.indexOf(selectedColis.statut);
    return currentIndex >= 0 ? statusHierarchy.slice(currentIndex + 1) : [];
  };
  ...
  return (
    <div className="d-flex flex-column min-vh-100" style={{ backgroundColor: '#fff' }}>
                        ...
                        <Card className="shadow-sm mb-4" style={{ borderColor: '#e67e22' }}>
                          ...
                          <Card.Body style={{ backgroundColor: '#fff' }}>
                            ...
                            <div className="d-flex justify-content-end mt-3">
                              <Button 
                                variant="warning"
                                onClick={() => setShowSuiviModal(true)}
                                className="me-2"
      ...
      <Modal show={showSuiviModal} onHide={() => setShowSuiviModal(false)} size="lg">
        <Modal.Header closeButton style={{ backgroundColor: '#e67e22', color: 'white' }}>
          <Modal.Title>Mise à jour du suivi - {selectedColis?.code_suivi}</Modal.Title>
        </Modal.Header>
        <Modal.Body style={{ backgroundColor: '#fff' }}>
          <Form>
            <Row>
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Select 
                    ...
                    onChange={(e) => setNewSuivi({...newSuivi, statut: e.target.value})}
                    required
                  >
              ...
              {['En livraison', 'Livré'].includes(newSuivi.statut) && (
                <>
                  <Col md={12}>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control 
                        type="text" 
                        value={newSuivi.location}
                        onChange={(e) => setNewSuivi({...newSuivi, location: e.target.value})}
                        placeholder="Adresse ou lieu actuel"
                      />
                    </Form.Group>
                  </Col>
                  <Col md={6}>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control 
                        type="number" 
                        step="any"
                        value={newSuivi.latitude || ''}
                        onChange={(e) => setNewSuivi({...newSuivi, latitude: e.target.value ? parseFloat(e.target.value) : null})}
                        placeholder="Ex: 4.0511"
                        min={CAMEROON_BOUNDS.getSouth()}
                        max={CAMEROON_BOUNDS.getNorth()}
                      />
                    </Form.Group>
                  </Col>
                  <Col md={6}>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control 
                        type="number" 
                        step="any"
                        value={newSuivi.longitude || ''}
                        onChange={(e) => setNewSuivi({...newSuivi, longitude: e.target.value ? parseFloat(e.target.value) : null})}
                        placeholder="Ex: 9.7679"
                        min={CAMEROON_BOUNDS.getWest()}
                        max={CAMEROON_BOUNDS.getEast()}
                      />
                    </Form.Group>
                  </Col>
                </>
        ...
        <Modal.Footer style={{ backgroundColor: '#fff' }}>
          <Button variant="secondary" onClick={() => setShowSuiviModal(false)}>
            Annuler
          </Button>
          <Button 
            variant="warning" 
            onClick={handleAddSuivi} 
            disabled={loading.submit || !newSuivi.statut}
            style={{ backgroundColor: '#e67e22', borderColor: '#e67e22', color: 'white' }}
          >
            {loading.submit ? (
...
export default SuiviColisReceptionnistPage;